...
def _print_results(result: pd.DataFrame) -> None:
    ...
    for i, row in result.iterrows():
        ...
        # Category — show all approaches + consensus
        approaches = [("Leaf-first", "leaf_first"), ("Two-step", "two_step"), ("Keyword", "keyword")]
        cat_rows = [
            (lbl, _val(row.get(f"img_cat_{key}_path")), _val(row.get(f"img_cat_{key}_id"))) for lbl, key in approaches
        ]
        if any(p for _, p, _ in cat_rows):
            cat_table = Table(box=box.SIMPLE, show_header=True, pad_edge=False)
            cat_table.add_column("Approach", style="dim", width=12)
            cat_table.add_column("Category")
            cat_table.add_column("ID", style="dim", width=8)
            for lbl, path, cat_id in cat_rows:
                cat_table.add_row(lbl, path or "[red]failed[/]", str(cat_id or ""))
            consensus = _val(row.get("img_category_path"))
            console.print(
                Panel(
                    cat_table,
                    title="[bold]Google Taxonomy",
                    border_style="green",
                )
            )
            if consensus:
                console.print(f"  [bold]Consensus:[/] {consensus}  [dim](id: {row.get('img_category_id')})[/]")
