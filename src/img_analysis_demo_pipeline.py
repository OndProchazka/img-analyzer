...
@step
def categorize(image_url: str) -> dict:
    """Analyze a single product image URL and return the enriched result dict (without taxonomy)."""
    if not image_url:
    ...
    return analyzer.analyze_image(image_url)
...
def img_analysis_demo_pipeline(
    ...
    """HTTP-deployable pipeline: accepts image_url and returns analysis dict."""
    return categorize(image_url=image_url)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "image_url",
        nargs="?",
        default="http://img0.fbcdn.cz/tedi/api/stored/12/e5/12e5e78ed6081c2997564f00b8372066.png",
    )
    args = parser.parse_args()

    img_analysis_demo_pipeline(image_url=args.image_url)
